...
class Router{
	...
	/**
     * Get The Current Page Field Value If Availabel
     * @var string
     */
	...
	function run($url){
		...
		if(class_exists($controller_name,true)){
			...
			//display the view as a subpage without the layout
			if($this->is_partial_view==true){
				$controller->view->is_partial_view=true;
	...
	function page_not_found($msg="We are sorry, the page you've requested is not available"){
		$view =new BaseView();
		$view->render("errors/error_404.php",$msg,"default_layout.php");
		exit;
	}
}