...
define('CACHE_DIR_NAME',		 'cached/');
define('CACHE_DIR',				UPLOAD_DIR . CACHE_DIR_NAME);
...
$arrPaths=explode("/",trim(ROOT,"/"));
$docRootName=$arrPaths[count($arrPaths)-1];

$image	=trim($image,"/");
if(strpos($image,$docRootName)===0){
	$image=substr($image,strlen($docRootName));
}
